...
export function createTextures(scene) {
  ...
  // Ходячий. Спина — синусоида с пиком посередине, тем же приёмом, что и
  // иголки ёжика: силуэт должен читаться на бегу, раньше, чем игрок успеет
  // разглядеть чешую. Горб к тому же обещает клубок — из такой спины он и
  // сворачивается.
  const спина = (t) => 40 - Math.sin(t * Math.PI) * 24; // t идёт от хвоста к морде

  g.fillStyle(0x8a6760, 1);
  g.lineStyle(4, 0x5d423c, 1);
  g.fillEllipse(12, 38, 26, 11); // хвост волочится
  g.strokeEllipse(12, 38, 26, 11);

  const горб = [];
  for (let i = 0; i <= 16; i++) горб.push({ x: 12 + (i / 16) * 48, y: спина(i / 16) });
  горб.push({ x: 60, y: 47 }, { x: 12, y: 47 });
  g.fillPoints(горб, true);
  g.strokePoints(горб, true);

  // Брюшко светлее спины: без него горб сливается в одно тёмное пятно.
  g.fillStyle(0xbfa29a, 1);
  g.lineStyle(3, 0x5d423c, 1);
  g.fillEllipse(36, 42, 46, 15);
  g.strokeEllipse(36, 42, 46, 15);

  // Лапки — короткие и толстые, как у настоящего панголина.
  g.fillStyle(0x6f524c, 1);
  g.lineStyle(3, 0x452f2b, 1);
  [22, 34, 46].forEach((x) => {
    g.fillRoundedRect(x, 44, 10, 11, 5);
    g.strokeRoundedRect(x, 44, 10, 11, 5);
  });

  // Чешуя ложится по той же синусоиде: два ряда внахлёст, вдоль горба.
  for (let i = 0; i < 10; i++) {
    const t = (i + 0.5) / 10;
    чешуйка(12 + t * 48 - 7, спина(t) - 2, 14, 10, i % 2 === 0);
  }
  for (let i = 0; i < 8; i++) {
    const t = (i + 0.5) / 8;
    чешуйка(15 + t * 42 - 6, спина(t) + 8, 12, 9, i % 2 === 1);
  }

  // Морда: вытянутый конус вправо, носик тёмной каплей.
  g.fillStyle(0xbfa29a, 1);
  g.lineStyle(3, 0x5d423c, 1);
  g.fillEllipse(64, 38, 30, 15);
  g.strokeEllipse(64, 38, 30, 15);
  g.fillStyle(0x452f2b, 1);
  g.fillCircle(77, 39, 4);
  eyes(57, 57, 32, 4); // глаз один: смотрит вбок
