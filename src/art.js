...
export function createTextures(scene) {
  ...
  // ── Панголин ──────────────────────────────────────────────────────────
  //
  // Четыре вида на одном холсте 96×56: два кадра бега, клубок и оглушённый.
  // Холст у всех один, иначе при смене вида спрайт дёргался бы: центр
  // остаётся на месте, а размеры холста меняются.
  //
  // Чешуя — главная его примета, поэтому она везде одна: округлые пластинки
  // внахлёст, светлее к спине и темнее к брюху.
  ...
  // Спина — синусоида с пиком посередине, тем же приёмом, что иголки ёжика:
  // силуэт должен читаться на бегу, раньше, чем игрок разглядит чешую. Горб
  // к тому же обещает клубок — из такой спины он и сворачивается.
  const спина = (t) => 34 - Math.sin(t * Math.PI) * 18; // t от хвоста к морде

  // Хвост длинный и тяжёлый: у панголина он почти в половину зверя и на бегу
  // качается. Им он и уравновешивает горб.
  const хвост = (взмах) => {
    // Длинный клин от туловища к острому кончику: у панголина хвост почти в
    // половину зверя, и держит он его низко, волоча по земле.
    const пласт = [
      { x: 0, y: 45 + взмах },
      { x: 8, y: 37 + взмах },
      { x: 22, y: 32 },
    ...
    g.fillPoints(пласт, true);
    g.strokePoints(пласт, true);
    // Чешуя идёт и по хвосту: у панголина он покрыт ею целиком, а к кончику
    // пластинки мельчают — так виден сам клин, а не полоска.
    for (let ряд = 0; ряд < 2; ряд++) {
      for (let i = 0; i < 4; i++) {
        const t = i / 3;
        чешуйка(
          3 + i * 9,
          38 - t * 6 + ряд * 6 + взмах * (1 - t),
          8 + t * 5,
          6 + t * 4,
          (i + ряд) % 2 === 0
        );
  ...
  // Ножки: два кадра бега. Разная длина — это поднятая и опущенная лапа,
  // а не ошибка: по ней и видно, что зверь перебирает ногами.
  const ножки = (расставлены) => {
    g.fillStyle(0x6f524c, 1);
    g.lineStyle(3, 0x452f2b, 1);
    const пары = расставлены
      ? [[34, 13], [45, 9], [59, 9], [70, 13]]
      : [[38, 9], [48, 13], [58, 13], [67, 9]];
    for (const [x, h] of пары) {
      g.fillRoundedRect(x, 40, 9, h, 4);
      g.strokeRoundedRect(x, 40, 9, h, 4);
    }
  };

  // Туловище: горб с чешуёй и светлое брюшко, приподнятое над землёй, чтобы
  // ноги было видно целиком.
  const туловище = () => {
    const горб = [];
    for (let i = 0; i <= 16; i++) горб.push({ x: 28 + (i / 16) * 48, y: спина(i / 16) });
    горб.push({ x: 76, y: 42 }, { x: 28, y: 42 });
    g.fillStyle(0x8a6760, 1);
    g.lineStyle(4, 0x5d423c, 1);
    g.fillPoints(горб, true);
    g.strokePoints(горб, true);

    g.fillStyle(0xbfa29a, 1);
    g.lineStyle(3, 0x5d423c, 1);
    g.fillEllipse(52, 38, 46, 14);
    g.strokeEllipse(52, 38, 46, 14);

    for (let i = 0; i < 10; i++) {
      const t = (i + 0.5) / 10;
      чешуйка(28 + t * 48 - 7, спина(t) - 2, 14, 10, i % 2 === 0);
    }
    for (let i = 0; i < 8; i++) {
      const t = (i + 0.5) / 8;
      чешуйка(31 + t * 42 - 6, спина(t) + 8, 12, 9, i % 2 === 1);
    }
  };
  ...
  ножки(true);
  морда();
  bake(g, 'pangolin', 96, 56);

  хвост(3);
  туловище();
  ножки(false);
  морда();
  bake(g, 'pangolin-run', 96, 56);

  // Клубок: та же чешуя, свёрнутая кольцами. Внизу виден краешек морды —
  // чтобы игрок понимал, что катится тот же зверь, а не камень.
  g.fillStyle(0x8a6760, 1);
  g.lineStyle(4, 0x5d423c, 1);
  g.fillCircle(48, 30, 25);
  g.strokeCircle(48, 30, 25);
  g.fillStyle(0xbfa29a, 1);
  g.lineStyle(3, 0x5d423c, 1);
  g.fillEllipse(48, 42, 26, 13); // подвёрнутая морда
  g.strokeEllipse(48, 42, 26, 13);
  for (let ряд = 0; ряд < 3; ряд++) {
    const r = 20 - ряд * 6;
    const сколько = 8 - ряд * 2;
    for (let i = 0; i < сколько; i++) {
      const a = (i / сколько) * Math.PI * 2 + ряд * 0.4;
      чешуйка(48 + Math.cos(a) * r - 6, 30 + Math.sin(a) * r - 4, 12, 9, ряд % 2 === 0);
    }
  }
  bake(g, 'pangolin-ball', 96, 56);

  // Оглушённый: лежит на боку, лапки кверху, глаза крестиками.
  хвост(0);
  g.fillStyle(0x8a6760, 1);
  g.lineStyle(4, 0x5d423c, 1);
  g.fillEllipse(50, 38, 52, 26);
  g.strokeEllipse(50, 38, 52, 26);
  g.fillStyle(0x6f524c, 1);
  g.lineStyle(3, 0x452f2b, 1);
  [36, 50, 62].forEach((x) => {
    g.fillRoundedRect(x, 20, 9, 12, 4);
  ...
  THEMES.forEach((theme, i) => {
    ...
    //
    // Картинка выше клетки на толщину каймы. Кайма идёт вниз от линии
    // поверхности, и у нижнего угла клетки эти пиксели выходят за её край.
    // Обрезанные, они давали клин: у одного края каймы полная толщина, у
    // другого ноль, и на каждом стыке она обрывалась ступенькой. Лишнее
    // свисает на клетку ниже, где всё равно земля.
    const КАЙМА = 14;
    const ВЫСОТА = 60 + КАЙМА;
    for (const вверх of [true, false]) {
      ...
      g.fillStyle(theme.grass, 1);
      g.fillPoints(скат(0, КАЙМА), true);
      g.fillStyle(theme.grassDark, 1);
      g.fillPoints(скат(12, 4), true);

      g.fillStyle(theme.dirtDark, 1);
      g.fillEllipse(вверх ? 44 : 16, 48, 12, 8);
      bake(g, `slope-${вверх ? 'up' : 'down'}-${i}`, 60, ВЫСОТА);
    }
