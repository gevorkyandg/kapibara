...
/**
 * ДЛЯ ТЕСТОВ. Общий сброс прогресса по метке из сборки.
 *
 * Зачем. Прогресс живёт в браузере игрока, и дотянуться до чужого нельзя ни
 * через GitHub, ни через базу — статистика только принимает записи. Чтобы
 * проверять новые этапы на чистых игроках, игра сама обнуляет прогресс, когда
 * видит незнакомую метку.
 *
 * Как. В config.js лежит СБРОС_ПРОГРЕССА. Пустая строка — не сбрасывать.
 * Любая другая срабатывает ровно один раз у каждого игрока: метка тут же
 * записывается в сохранение, и второй раз та же строка уже ничего не делает.
 * Понадобится сбросить снова — меняем строку, обычно на новую дату.
 *
 * Звук не трогаем: он к прогрессу отношения не имеет, а сбрасывать его —
 * значит каждый раз включать музыку тому, кто её выключил.
 *
 * @returns {boolean} был ли сброс прямо сейчас
 */
export function сброситьЕслиПросили() {
  if (!СБРОС_ПРОГРЕССА || data.сброс === СБРОС_ПРОГРЕССА) return false;
