...
export function startWatchdog(game, показывать = true) {
  ...
  // Полоска молчит, пока всё хорошо. Раньше она висела всегда и закрывала
  // собой монеты, время и жизни — то есть ровно ту информацию, ради которой
  // на неё и смотрят. Теперь появляется только на поломке и через десять
  // секунд убирается сама.
  let спрятать = null;
  const перерисовать = () => {
    if (!полоска || !тревога) return;
    полоска.textContent = [состояние(), ...строки].join('\n');
    полоска.style.background = 'rgba(180,60,40,.94)';
    полоска.style.display = 'block';
    clearTimeout(спрятать);
    спрятать = setTimeout(() => {
