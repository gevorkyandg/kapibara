...
export class BootScene extends Phaser.Scene {
  ...
  async create() {
    ...
    // с картой. Читаем их только когда игру открыл редактор — иначе забытая
    // в браузере запись превращала бы обычную игру в вечный первый уровень.
    const изРедактора =
      import.meta.env.DEV && new URLSearchParams(location.search).has('этап');
    if (изРедактора) {
      try {
