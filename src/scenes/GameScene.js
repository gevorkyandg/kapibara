...
// осы (460), чтобы предупреждение всегда успевало прийти первым: увидеть
// табличку игрок должен раньше, чем его увидит рой.
const HIVE_SIGN_AHEAD = 610;
...
export class GameScene extends Phaser.Scene {
  ...
  addMonster(symbol, x, y) {
    ...
    // Колючего не берёт касание — только рогатка. У панголина то же поле
    // включается на время клубка, поэтому здесь его нужно завести явно.
    m.колючий = Boolean(type.колючий);
    m.броня = false;
...
/**
  ...
  handleMovement(time, onFloor) {
    ...
    // В стену не толкаемся вовсе.
    //
    // Толку от этого нет — сквозь стену не пройдёшь, — а вреда два. Прыжок у
    // стены гаснет: за кадр тело въезжает в неё на несколько пикселей, и
    // физика видит перекрытие сразу по двум осям, а разводит по вертикали. И
    // второе: падая вдоль стены с зажатым «вперёд», капибара продавливается
    // внутрь блока и садится на крышу блока этажом ниже. Стоит в стене, а
    // отпустишь ход — проваливается.
    //
    // «Стена» здесь — то, через что нельзя переступить. Мелкая ступенька
    // склона стеной не считается: в неё толкаться как раз нужно, иначе не
    // сработает заход на неё, и капибара встанет перед скатом намертво.
    const шаг = right ? 1 : left ? -1 : 0;
    const вСтену = this.стенаВпереди(шаг);
  ...
  touchEnemy(enemy) {
    ...
    // Есть монстры, которых касание не берёт вовсе.
    //
    // Ёжик — потому что он весь в иглах: врезаться в него боком и тем убить
    // нельзя, платит только герой. Панголин в клубке — потому что он в этот
    // миг уже не монстр, а снаряд; уязвимым он станет там, где остановится:
    // у обрыва или оглушённым после удара о стену.
    if (enemy.броня || enemy.колючий) {
    ...
    // Столкновение боком — размен: монстр гибнет, но и жизнь уходит.
    //
    // Гибель монстра без платы превращала бы его в подбираемое: можно было
    // просто идти вперёд, вычищая телом весь этап. Плата возвращает выбор —
    // обойти, перепрыгнуть или разменять жизнь на проход там, где иначе не
    // пройти вовсе.
    //
  ...
  /**
   * Впереди стена, а не ступенька склона.
   *
   * Отличаем по тому же признаку, по которому работает заход на ступеньку:
   * если над препятствием у самых ног есть просвет в несколько пикселей, это
   * ступенька и её переступают. Если сплошное — стена.
   */
  стенаВпереди(dir) {
    if (!dir) return false;
    const тело = this.player.body;
    const впереди = this.player.x + dir * (тело.halfWidth + 3);

    // Смотрим по всей высоте тела: у ямы стена начинается ниже пояса, а у
    // низкого свода — наоборот, только сверху.
    const есть =
      this.isSolidAtPixel(впереди, тело.center.y) ||
      this.isSolidAtPixel(впереди, тело.top + 6) ||
      this.isSolidAtPixel(впереди, тело.bottom - 6);
    if (!есть) return false;

    const предел = Math.ceil(SLOPE_STEP) + 4;
    for (let h = 1; h <= предел; h++) {
      if (!this.isSolidAtPixel(впереди, тело.bottom - 1 - h)) return false;
    }
    return true;
  }
