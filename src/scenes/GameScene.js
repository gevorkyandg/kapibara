...
export class GameScene extends Phaser.Scene {
  ...
  выбратьМестоВалуна(целX) {
    ...
    for (const dir of стороны) {
      for (const даль of BOULDER_DISTANCES) {
        ...
        if (this.путьВниз(опора.col, цельCol, опора.row)) return место;
        запасной.push(место);
      }
    }
  ...
  /** Не поднимается ли земля выше места появления по дороге к игроку. */
  ...
  updateBoulders(time, delta) {
    ...
    for (const run of this.boulderRuns) {
      ...
      // Камень появляется перед игроком или за ним, на постоянном удалении и
      // выше земли — и всё. Разбор рельефа отсюда убран намеренно: он давал
      // то камень внутри склона, то отказ ставить камень вовсе. Пусть просто
      // падает на то, что под ним, и катится.
      const откуда = this.player.x;
      const место = this.выбратьМестоВалуна(откуда);
      if (!место) {
