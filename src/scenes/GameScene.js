...
/**
  ...
  handleMovement(time, onFloor) {
    ...
    // В воздухе в стену не толкаемся.
    //
    // Толкаться там нельзя дважды. Во-первых, за кадр тело въезжает в стену
    // на несколько пикселей, и если в тот же кадр начался прыжок, Arcade
    // видит перекрытие сразу по двум осям и разводит тела по вертикали —
    // прыжок гаснет в первом же кадре. Во-вторых, падая вдоль стены с
    // зажатым «вперёд», капибара продавливается внутрь блока и садится на
    // крышу блока этажом ниже: стоит в стене, а отпустишь ход — проваливается.
    //
    ...
    // Кадр, в котором прыжок вот-вот начнётся, считается тем же случаем: там
    // капибара ещё на земле, а перекрытие со стеной уже губит прыжок.
    const прыгнёмСейчас =
      time - this.jumpBufferedAt < 130 && (onFloor || time - this.lastFloorTime < 110);
    const вСтену =
      шаг !== 0 &&
      (!onFloor || прыгнёмСейчас) &&
