...
export const MONSTERS = {
  ...
  p: {
    ...
    update(scene, m, time) {
      ...
      // маршруту или возвращается на него после неудачного заезда.
      //
      // Но только если герой стоит на той же земле. «Один уровень» — это
      // общий пол, а не близкая высота: с уступа игрок дразнил панголина,
      // тот разгонялся, разбивался о стену внизу и ложился оглушённым, ничем
      // герою не грозя. Такая добыча достаётся даром.
      //
      // Высоту героя берём не текущую, а последней опоры: в прыжке он на
      // мгновение оказывается «выше уровнем», и панголин иначе терял бы цель
      // ровно в тот миг, когда она к нему летит.
      const полГероя = scene.lastFloorY ?? игрок.body.bottom;
      const наОдномПолу = Math.abs(полГероя - m.body.bottom) < 40;
      if (Math.abs(игрок.x - m.x) <= m.sight && наОдномПолу) {
        m.state = 'alert';
  ...
  z: {
    ...
    // капибару, пока та рядом.
    //
    // Сверху её топчут, как всех прочих: колючек у неё нет, и прыжок на
    // голову — честный ответ на выпад. Раньше она кусала и в полёте, отчего
    // прицельный прыжок наказывался наравне с зевком.
    key: 'snake',
    xp: XP.monster.hard,
    stompable: true,
    ground: false,
